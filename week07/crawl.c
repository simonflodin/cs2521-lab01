...
int main (int argc, char **argv)
{
	...
	Graph g = newGraph(sizeof(maxURLs));
	Set seen = newSet();
	char *URL;
	char res[BUFSIZE];
	int position = 0;
	handle = url_fopen (firstURL, "r");
	while(!emptyQueue(toDo) && nVertices(g) < sizeof(maxURLs)){
		URL = leaveQueue(toDo);
		if(strstr(URL, "unsw") != NULL) continue;
		while (!url_feof (handle)) {
			while ((position = GetNextURL (buffer, URL, res, position)) > 0) {
				if(nVertices(g) < sizeof(maxURLs) || (isElem(seen, URL) && isElem(seen, res))){
					addEdge(g, URL, res);
				}
				if(!isElem(seen, res)){
					insertInto(seen, res);
