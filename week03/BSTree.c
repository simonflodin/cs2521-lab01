...
void BSTreeLevelOrder (BSTree t)
{
	//Init queue
	Queue tree = newQueue();
	//add T's node to queue
	QueueJoin(tree, t);
	while(QueueIsEmpty(tree) == false){
		Item head = QueueLeave(tree);
		if(head == NULL){
			continue;
		}
		printf("%d ", head->value);
		if(head->left != NULL){
			QueueJoin(tree, head->left);
		}
		if(head->right != NULL){
			QueueJoin(tree, head->right);
		}
	}
	dropQueue(tree);
}
...
int BSTreeNumLeaves (BSTree t)
{
	...
	if(t == NULL){
		...
		RHleaves = BSTreeNumLeaves(t->right);
		LHleaves = BSTreeNumLeaves(t->left);
	}
	return LHleaves + RHleaves;
}
