...
int findPath (Graph g, Vertex src, Vertex dest, int max, int *path)
{
	...
	QueueJoin(priorityQueue, src);
	while(found == 0 && !QueueIsEmpty(priorityQueue)){
		Item v = QueueLeave(priorityQueue);
		if(v == dest){
			found = 1;
		}else{
	...
	// 	path[k] = ReversePath[counter];
	// 	k++;
	// 	counter--;
	// }
	dropQueue(priorityQueue);
	return i+1;
